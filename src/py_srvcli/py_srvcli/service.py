...
class MinimalService(Node):
    ...
    def add_callback(self, request, response):
        response.sum = request.a + request.b
        self.get_logger().info('Add request: a: %d, b: %d, result: %d' % (request.a, request.b, response.sum))
        return response

    def subtract_callback(self, request, response):
        response.sum = request.a - request.b
        self.get_logger().info('Subtract request: a: %d, b: %d, result: %d' % (request.a, request.b, response.sum))
        return response

    def multiply_callback(self, request, response):
        response.sum = request.a * request.b
        self.get_logger().info('Multiply request: a: %d, b: %d, result: %d' % (request.a, request.b, response.sum))
        return response

    def divide_callback(self, request, response):
        # 나누기 연산 시, 0으로 나누는 것을 방지해야 합니다.
        if request.b == 0:
            self.get_logger().error('Divide request failed: Division by zero')
            response.sum = 0  # 오류를 처리할 기본값 설정
        else:
            response.sum = request.a / request.b
            self.get_logger().info('Divide request: a: %d, b: %d, result: %f' % (request.a, request.b, response.sum))
        return response
        return response
